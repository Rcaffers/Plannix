...
const directory = await mkdtemp(path.join(tmpdir(), 'plannix-palette-browser-'));
try {
  await build({ configFile: false, logLevel: 'error', define: { 'process.env.NODE_ENV': '"production"' }, esbuild: { jsx: 'automatic' },
    build: { outDir: directory, emptyOutDir: false, lib: {
      entry: 'src/components/ClassPlacementPalette.browser-test.jsx', formats: ['iife'], name: 'PaletteTest', fileName: () => 'test.js', cssFileName: 'test',
    } } });
  ...
  const dom = await new Promise((resolve, reject) => {
    const child = spawn(chrome, ['--headless', '--disable-background-networking', '--disable-component-update',
      ...
      `--user-data-dir=${path.join(directory, 'profile')}`, '--dump-dom', '--timeout=10000', `file://${directory}/index.html`],
