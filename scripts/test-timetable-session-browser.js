...
const directory = await mkdtemp(path.join(tmpdir(), 'plannix-session-browser-'));
try {
  await build({ plugins: [{ name: 'session-provider-boundaries', enforce: 'pre', resolveId(source, importer) {
    if (importer?.endsWith('/TimetableSessionContext.jsx') &&
        ['./AcademicYearContext', './ClassContext', './TimetableLayoutContext', '../utils/timetableSessionApi'].includes(source)) {
      return path.resolve('src/context/TimetableSessionContext.browser-test.jsx');
    }
    ...
    build: { outDir: directory, emptyOutDir: false, lib: {
      entry: 'src/context/TimetableSessionContext.browser-test.jsx', formats: ['iife'], name: 'SessionProviderTest', fileName: () => 'test.js', cssFileName: 'test',
    } } });
  await writeFile(path.join(directory, 'index.html'), '<!doctype html><html><head></head><body><script src="test.js"></script></body></html>');
